...
export function getRulesSection(cwd: string, settings?: SystemPromptSettings): string {
	...
	const toolPacingRule = isTerminalPromptProfile(settings)
		? "- Do not pause after every successful tool call. Continue working until you hit a real blocker, need approval or user input, or are ready to finish the task."
...
- ${toolPacingRule.slice(2)}${settings?.isStealthModel ? getVendorConfidentialitySection() : ""}`
}
